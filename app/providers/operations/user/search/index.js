...
class UserSearchOperation {
  ...
  async perform (ws, searchParams) {
    ...
    const limit = searchParams.limit > MAIN_CONSTANTS.LIMIT_MAX ? MAIN_CONSTANTS.LIMIT_MAX : searchParams.limit || MAIN_CONSTANTS.LIMIT_MAX
    ...
    const usersSearchResult = users.map(user => ({ _id: user._id.toString(), login: user.login }))
