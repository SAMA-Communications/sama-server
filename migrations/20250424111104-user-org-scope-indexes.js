export const up = async (db, client) => {
    ...
    await db.collection("users").createIndex({ organization_id: 1, email: 1 }, { unique: true, partialFilterExpression: { email: { $gt: '' } } })
    await db.collection("users").createIndex({ organization_id: 1, phone: 1 }, { unique: true, partialFilterExpression: { phone: { $gt: '' } } })
  }
