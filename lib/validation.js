...
function validateMessageBody(vParams) {
  if (
    !(
      (vParams.body && vParams.body.length) ||
      vParams.attachments?.length !== 0
    )
  ) {
    throw new Error(ERROR_STATUES.MESSAGE_CONTENT_EMPTY.message, {
      cause: ERROR_STATUES.MESSAGE_CONTENT_EMPTY,
    });
  }
}
