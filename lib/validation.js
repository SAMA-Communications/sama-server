...
// File -->>>
function validateFileId(vParams) {
  if (!vParams.id || vParams.id.length === 0) {
    throw new Error(ERROR_STATUES.FILE_ID_MISSED.message, {
      cause: ERROR_STATUES.FILE_ID_MISSED,
    });
  }
}
function validateFileDownloadUrl(vParams) {
  if (!vParams.url || typeof vParams.url !== "string") {
    throw new Error(ERROR_STATUES.FILE_URL_NOT_FOUND.message, {
      cause: ERROR_STATUES.FILE_URL_NOT_FOUND,
    });
  }
}
//  Messages -->>>
