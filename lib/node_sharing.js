...
import ip from "ip";
import os from "os";
import { createToNodeSocket, getClusterPort } from "../cluster/cluster_manager.js";
...
class ClusterManager {
  ...
  async retrieveExistingClusterNodes() {
    ...
    nodeList.forEach(async (n) => {
      ...
      if (getClusterPort() !== n.port && this.hostname < n.hostname)
  ...
  async startSyncingClusterNodes() {
    ...
    const storeCurrentNode = async ({ ip_address, hostname, port, users_count }) => {
      if (await ClusterNode.findOne({ ip_address, hostname, port })) {
        ...
        const newNode = new ClusterNode({ ip_address, hostname, port, users_count });
        await newNode.save();
      }
    };
