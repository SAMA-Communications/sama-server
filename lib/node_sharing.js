...
import ip from "ip";
import os from "os";
import { createToNodeSocket } from "../cluster/cluster_manager.js";
  ...
  async retrieveExistingClusterNodes() {
    ...
    nodeList.forEach(async (n) => {
      ...
      if (this.port !== n.port && this.hostname < n.hostname)
        //TODO: replace next two lines
        // await createToNodeSocket(`ws://${n.ip_address}:${n.port}/`);
        await createToNodeSocket(`ws://localhost:${n.port}/`);
    });
  }
